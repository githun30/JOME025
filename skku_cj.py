...
st.write('''###### 👉 이후로도 전반적인 우상향 형태로 증가하는 추세를 보임.''')
st.write('''###### 👉 시민이 언론조정 등의 분쟁이 발생하게 되면 피해가 막심하므로 언론은 사회적 책임을 갖고 이를 최소화할 의무가 있음.''')
st.write("")
st.write("")
...
st.text('출처: 중앙일보')
st.write('''###### 👉 피해구제를 목적으로 하는 법률 목적을 기반으로, 개정안 주요 내용은 피해 최소화(열람 차단 청구권), 손해배상액에 초점이 맞춰져 있음(이혜리, 2021).''')
st.write('''###### 👉 그러나 성추문, 비리범죄와 같은 사회고발적 문제에 대한 언론의 취재와 보도를 제한할 수 있다는 우려 제기됨(유성운, 2011).''')
st.write("")
...
st.write('''###### 👉 언론미디어 특별위원회(언론특위)가 구성되었으나, 국정감사 및 20대 대통령 선거 등의 이슈로 유의미한 결과를 내지는 못함(강한들, 2021; 정철운, 2022).''')
st.write("")
